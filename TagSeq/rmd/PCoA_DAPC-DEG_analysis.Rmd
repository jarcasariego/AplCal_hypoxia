...
pcoa <- ggplot(mds, aes(color = treatment, shape = tissue)) +
  ...
  scale_alpha_manual(values = c(1, 0), name = "cohort", labels = c("naive", "pre-exposed")) +
...
pcoa <- ggplot(mds, aes(color = treatment, shape = tissue)) +
  ...
  labs(x = paste0("PC1 [", vexpl[1],"%]"), y = paste0("PC2 [", vexpl[2],"%]")) +
  theme_custom() +
  theme(legend.spacing = unit(0, "cm"), legend.key.size = unit(1, 'lines')) +
...
pcoa <- ggplot(mds, aes(color = treatment, shape = btch)) +
  ...
  geom_point(size = 2, aes(x = c1, y = c2), show.legend = FALSE) +
  geom_point(size = 0.7, aes(x = `1`, y = `2`)) +
  scale_shape_discrete(name = "cohort", labels = c("naive", "pre-exposed")) +
  scale_color_discrete(name = "treatment") +
  labs(x = paste0("PC1 [", vexpl[1],"%]"), y = paste0("PC2 [", vexpl[2],"%]")) +
  theme_custom() +
  theme(legend.spacing = unit(0, "cm"), legend.key.size = unit(1, 'lines')) +
...
# Run DESeq pipeline 
design(dds_60) <- formula(~ treatment)
dsr2 <- DESeq(dds_60)
...
```{r get_DEgenes}
DE <- DE %>%
  mutate(sig = map(dsr, ~ rownames_to_column(data.frame(.[which(.$padj < 0.01), ]), "gene")),
...
# Run DESeq pipeline 
design(dds_71) <- formula(~ treatment)
dsr2 <- DESeq(dds_71)
...
```{r run_DESeq_for Abdominal all, warning=FALSE}
# Run DESeq pipeline
...
```{r run_DESeq_together abdominal all}
# Run DESeq pipeline 
...
```{r run_DESeq_for Pleural/Pedal all, warning=FALSE}
# Run DESeq pipeline
...
Then, for both batches together
```{r run_DESeq_together Pleural & Pedal all}
# Run DESeq pipeline 
...
### Get significant DEGs
```{r get_DEgenes Pleural & Pedal all}
DE_Pp <- DE_Pp %>%
  mutate(sig = map(dsr, ~ rownames_to_column(data.frame(.[which(.$padj < 0.01), ]), "gene")),
          up = map(sig, ~ filter(., log2FoldChange > 0)),
        down = map(sig, ~ filter(., log2FoldChange < 0))) 
...
Compare batches, tissues, and per treatment (how different is their response)
...
# Define group contrasts
group.contrasts <- tibble(num = c("60"),
                          den = c("71"))

# Get DESeq results for all batch contrasts for each treatment
DE_A_batch <- crossing(treatment = c("Control", "Hyp_T2h","Hyp_T6h","LtH_6","LtH_7","Reox","Recovery"), group.contrasts) %>%
  mutate(dsr = pmap(list(treatment, num, den), function(x, y, z) {
    results(dsr1, contrast = c("batch.group", paste0(c(y, z), ".", x)))}))
```


### Get significant DEGs
```{r get_DEgenes Abdominal}
DE_A_batch <- DE_A_batch %>%
  mutate(sig = map(dsr, ~ rownames_to_column(data.frame(.[which(.$padj < 0.01), ]), "gene")),
          up = map(sig, ~ filter(., log2FoldChange > 0)),
        down = map(sig, ~ filter(., log2FoldChange < 0))) 

# Generate logP values for all differential expression contrasts
DE_A_batch <- DE_A_batch %>% mutate(logP = map(dsr, ~ data.frame(
  gene = rownames(data.frame(.)),
  logP = -log10(data.frame(.)$pvalue) * sign(data.frame(.)$log2FoldChange))))

# Count number of differentially expressed genes within each batch, and overall, for each contrast
DE_A_batch_tab <- DE_A_batch %>%
  filter((num == "60" & den == "71")) %>%
  mutate(nsig = map_dbl( sig, ~ nrow(.)),
          nup = map_dbl(  up, ~ nrow(.)),
          ndn = map_dbl(down, ~ nrow(.)),
          `DEGs [up, down]` = paste0(nsig, " [", nup, ", ", ndn, "]")) %>%
  select(treatment, num, den, `DEGs [up, down]`) %>%
  spread(treatment, `DEGs [up, down]`)

DE_A_batch_tab$contrast <- paste(DE_A_batch_tab$num, DE_A_batch_tab$den, sep = " vs ")
dek <- DE_A_batch_tab %>% select(c(10,3:9)) %>%
  knitr::kable(format = "markdown", caption = "Number of differentially expressed genes within individual genets and across all genets for each specified contrast. DEGs identified using DESeq with an adjusted p-value < 0.1. In brackets are the numbers of significantly up- and down-regulated genes.", row.names = )
...
# Get DESeq results for all batch contrasts for each treatment
DE_Pp_batch <- crossing(treatment = c("Control", "Hyp_T2h","Hyp_T6h","LtH_6","LtH_7","Reox","Recovery"), group.contrasts) %>%
  mutate(dsr = pmap(list(treatment, num, den), function(x, y, z) {
    results(dsr1, contrast = c("batch.group", paste0(c(y, z), ".", x)))}))
```
...
DE_Pp_batch <- DE_Pp_batch %>%
  mutate(sig = map(dsr, ~ rownames_to_column(data.frame(.[which(.$padj < 0.01), ]), "gene")),
          up = map(sig, ~ filter(., log2FoldChange > 0)),
        down = map(sig, ~ filter(., log2FoldChange < 0))) 

# Generate logP values for all differential expression contrasts
DE_Pp_batch <- DE_Pp_batch %>% mutate(logP = map(dsr, ~ data.frame(
  gene = rownames(data.frame(.)),
  logP = -log10(data.frame(.)$pvalue) * sign(data.frame(.)$log2FoldChange))))

# Count number of differentially expressed genes within each batch, and overall, for each contrast
DE_Pp_batch_tab <- DE_Pp_batch %>%
  filter((num == "60" & den == "71")) %>%
  mutate(nsig = map_dbl( sig, ~ nrow(.)),
          nup = map_dbl(  up, ~ nrow(.)),
          ndn = map_dbl(down, ~ nrow(.)),
...
DE_Pp_batch_tab$contrast <- paste(DE_Pp_batch_tab$num, DE_Pp_batch_tab$den, sep = " vs ")
dek <- DE_Pp_batch_tab %>% select(c(10,3:9)) %>%
  knitr::kable(format = "markdown", caption = "Number of differentially expressed genes within individual genets and across all genets for each specified contrast. DEGs identified using DESeq with an adjusted p-value < 0.1. In brackets are the numbers of significantly up- and down-regulated genes.", row.names = )
...
#graph DEGs between batches across all treatments

DEGs_A_batch <- DE_A_batch %>%
  unnest(sig) %>%
  mutate(contrast = paste(num, den, sep = ".")) %>%
  select(.,contrast, gene, treatment)
DEGs_A_batch_wide <- list(Control=pull(DEGs_A_batch[which(DEGs_A_batch$treatment=="Control"),2]),
                 Hyp_6h=pull(DEGs_A_batch[which(DEGs_A_batch$treatment=="Hyp_T6h"),2]),
                 Hyp_T6d=pull(DEGs_A_batch[which(DEGs_A_batch$treatment=="LtH_6"),2]),
                 Recovery=pull(DEGs_A_batch[which(DEGs_A_batch$treatment=="Recovery"),2]))
str(DEGs_A_batch_wide)
gv7 <- ggvenn(DEGs_A_batch_wide, fill_color = c("#0073C2FF", "#EFC000FF", "#868686FF", "#CD534CFF"),
  stroke_size = 0.5, set_name_size = 4, show_percentage = F, stroke_color = F 
  )
gv7
ggsave("figures/A_Between_batch_DEGs_vennDiagram.png", gv7, height = 3, width = 5)

DEGs_Pp_batch <- DE_Pp_batch %>%
  unnest(sig) %>%
  mutate(contrast = paste(num, den, sep = ".")) %>%
  select(.,contrast, gene, treatment)
DEGs_Pp_batch_wide <- list(Control=pull(DEGs_Pp_batch[which(DEGs_Pp_batch$treatment=="Control"),2]),
                 Hyp_6h=pull(DEGs_Pp_batch[which(DEGs_Pp_batch$treatment=="Hyp_T6h"),2]),
                 Hyp_T6d=pull(DEGs_Pp_batch[which(DEGs_Pp_batch$treatment=="LtH_6"),2]),
                 Recovery=pull(DEGs_Pp_batch[which(DEGs_Pp_batch$treatment=="Recovery"),2]))
str(DEGs_Pp_batch_wide)
gv8 <- ggvenn(DEGs_Pp_batch_wide, fill_color = c("#0073C2FF", "#EFC000FF", "#868686FF", "#CD534CFF"),
  stroke_size = 0.5, set_name_size = 4, show_percentage = F, stroke_color = F 
  )
gv8
ggsave("figures/Pp_Between_batch_DEGs_vennDiagram.png", gv8, height = 3, width = 5)

```


### DEG Similarities
Similarity of DEGs direction across tissue and contrasts
...
DEGs_A_batch <- DE_A_batch %>%
  unnest(sig) %>%
  mutate(contrast = paste(num, treatment, den, treatment, sep = ".")) %>%
  select(.,contrast, gene, log2FoldChange)
write_tsv(DEGs_A_batch, path = "output/DEGs_by_treat_batch_contrasts_Abdominal.tsv")

DEGs_Pp_batch <- DE_Pp_batch %>%
  unnest(sig) %>%
  mutate(contrast = paste(num, treatment, den, treatment, sep = ".")) %>%
  select(.,contrast, gene, log2FoldChange)
write_tsv(DEGs_Pp_batch, path = "output/DEGs_by_treat_batch_contrasts_PlePed.tsv")
```


#### DEGs in common between peak of exposure vs Ctrl contrasts (T6h vs Ctrol, LtH_6 vs Ctrl & LtH_7 vs Ctrl) 
in batch 60 abdominal
```{r DEGs peak 60A, fig.height = 8, fig.width = 8}
genesincommon_60A <- DE %>%
  unite("contrast", num, den, sep = ".") %>%
  filter(tissue == "A", contrast %in% c("Hyp_T2h.Control","Hyp_T6h.Control", "LtH_6.Control", "LtH_7.Control","Recovery.Control","Reox.Control")) %>%
  unnest(sig) %>%
  select(contrast, gene, log2FoldChange) %>%
...
# Get group log2fold changes for plotting
