...
# Separate by batch and run individual networks for batch

```{r separate by ganglia}

#keep only naive animals
sdat_60 <- sdat2[which(sdat2$batch == "60"),]
counts4wgcna_60 <-counts4wgcna[, which(colnames(counts4wgcna)%in%rownames(sdat_60))]
sdat_60$batch <- as.factor(sdat_60$tissue)
dds_4wgcna_60 <- DESeqDataSetFromMatrix(countData = counts4wgcna_60,
                              colData = sdat_60,
                              design = ~ treatment+tissue+treatment:tissue)
str(dds_4wgcna_60)
Vsd_60=vst(dds_4wgcna_A)
vsd.wg_60=assay(Vsd_60)
datt_60=t(vsd.wg_60) 

#keep only pre-exposed animals
sdat_71 <- sdat2[which(sdat2$batch == "71"),]
counts4wgcna_71 <-counts4wgcna[, which(colnames(counts4wgcna)%in%rownames(sdat_71))]
sdat_71$batch <- as.factor(sdat_71$tissue)
dds_4wgcna_71 <- DESeqDataSetFromMatrix(countData = counts4wgcna_71,
                              colData = sdat_71,
                              design = ~ treatment+tissue+treatment:tissue)
str(dds_4wgcna_71)
Vsd_71=vst(dds_4wgcna_71)
vsd.wg_71=assay(Vsd_71)
datt_71=t(vsd.wg_71) 

# assembling table of QUANTITATIVE traits for both batch datasets: dummy quantitative variables instead of categorical factor levels:
tissue=as.numeric(sdat_60$tissue=="A")
Control=as.numeric(sdat_60$treatment=="Control")
Hyp_T2h=as.numeric(sdat_60$treatment=="Hyp_T2h")
Hyp_T6h=as.numeric(sdat_60$treatment=="Hyp_T6h")
Reox=as.numeric(sdat_60$treatment=="Reox")
Recovery=as.numeric(sdat_60$treatment=="Recovery")
LtH_6=as.numeric(sdat_60$treatment=="LtH_6")
LtH_7=as.numeric(sdat_60$treatment=="LtH_7")
traits_60=data.frame(cbind(tissue,Control,Hyp_T2h,Hyp_T6h,Reox,LtH_6,LtH_7,Recovery))
traits_60

tissue=as.numeric(sdat_71$tissue=="A")
Control=as.numeric(sdat_71$treatment=="Control")
Hyp_T2h=as.numeric(sdat_71$treatment=="Hyp_T2h")
Hyp_T6h=as.numeric(sdat_71$treatment=="Hyp_T6h")
Reox=as.numeric(sdat_71$treatment=="Reox")
Recovery=as.numeric(sdat_71$treatment=="Recovery")
LtH_6=as.numeric(sdat_71$treatment=="LtH_6")
LtH_7=as.numeric(sdat_71$treatment=="LtH_7")
traits_71=data.frame(cbind(tissue,Control,Hyp_T2h,Hyp_T6h,Reox,LtH_6,LtH_7,Recovery))
traits_71

save(vsd.wg_60,sdat_60,datt_60,traits_60,file="output/data4wgcna_60.RData")
save(vsd.wg_71,sdat_71,datt_71,traits_71,file="output/data4wgcna_71.RData")
```
...
# Call the network topology analysis function
# *Decide if you want signed or unsigned network type
sft = pickSoftThreshold(datt_60, powerVector = powers, verbose = 5,networkType="signed")

# Plot the results:
sizeGrWindow(9, 5)
pdf("output/soft_threshold_signed_60.pdf",height=4, width=8)
par(mfrow = c(1,2));
cex1 = 0.9;
# Scale-free topology fit index as a function of the soft-thresholding power
plot(sft$fitIndices[,1], -sign(sft$fitIndices[,3])*sft$fitIndices[,2],
...
# 8 looks like the good threshold to use for 60

# Pleural/Pedal
powers = c(seq(from = 2, to=26, by=1))
# Call the network topology analysis function
# *Decide if you want signed or unsigned network type
sft = pickSoftThreshold(datt_71, powerVector = powers, verbose = 5,networkType="signed")

# Plot the results:
sizeGrWindow(9, 5)
pdf("output/soft_threshold_signed_71.pdf",height=4, width=8)
par(mfrow = c(1,2));
cex1 = 0.9;
# Scale-free topology fit index as a function of the soft-thresholding power
plot(sft$fitIndices[,1], -sign(sft$fitIndices[,3])*sft$fitIndices[,2],
...
# 10 looks like the good threshold to use for 71
...
load('output/1stPassModules_60.RData')
load('output/data4wgcna_60.RData')
