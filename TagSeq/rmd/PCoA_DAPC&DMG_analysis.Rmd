...
ggsave(filename = "figures/Fig.DAPC_71_GE.png",dapc.fig, width = 171, height = 110, units = "mm")
...
ggsave(filename = "figures/Fig.DAPC_60_GE.png",dapc.fig, width = 171, height = 110, units = "mm")
...
kogtables <- KOG %>%
  mutate(batch= case_when(batch == 60 ~ "captive parents", 
                           ...
                           batch == "all" ~ "all batches")) %>%
  mutate(batch = factor(batch, levels = c("captive parents", "wild parents", "all batches"))) %>%
  unnest(KOG) %>%
...
kogtable2 <- KOG %>%
  ...
  select(term, Hyp_T2h.Control, Hyp_T6h.Control, LtH_6.Control, LtH_7.Control, Recovery.Control, Reox.Control) %>%
...
KOG_abdominal <- KOGheatmap(
  select(kogtable2, Hyp_T2h.Control, Hyp_T6h.Control, LtH_6.Control, LtH_7.Control, Recovery.Control, Reox.Control),
  select(kogpval2, Hyp_T2h.Control, Hyp_T6h.Control, LtH_6.Control, LtH_7.Control, Recovery.Control, Reox.Control),
  labels_col = c(expression("2h_Hypoxia"~"vs."~"Control"),
                  expression("6h_Hypoxia"~"vs."~"Control"),
                  expression("6day_Hypoxia"~"vs."~"Control"),
                  expression("7day_Hypoxia"~"vs."~"Control"),
                  expression("Recovery"~"vs"~"Control"),
                  expression("Reoxigenation" ~"vs"~"Control")),
  fontsize = 8)

ggsave(filename = "figures/KOG_abdominal.png", KOG_abdominal, width = 112, height = 84.5, units = "mm")
```
...
ggsave(filename = "figures/KOG_PleuralPedal.png", KOG_Pp, width = 112, height = 84.5, units = "mm")
```