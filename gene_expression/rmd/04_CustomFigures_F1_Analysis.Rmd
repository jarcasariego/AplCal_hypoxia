...
top_ha = HeatmapAnnotation(
    "lab_up" = anno_barplot(comb_size(m_list[[3]]), 
        gp = gpar(fill = "#B31B21"), height = unit(2.5, "cm"), ylim = c(0,350)), 
    "lab_down" = anno_barplot(comb_size(m_list[[2]]), 
    ...
    gap = unit(1, "mm"), annotation_name_side = "left", annotation_name_rot = 90, annotation_name_offset = unit(10, "mm"))
...
df <- hyp_DEGs_L %>%
  ...
  mutate(Category = factor(ifelse(significance_Lab & significance_Wild, "Sig.Both",
                 ifelse(significance_Lab, "Sig.Naive",
                 ifelse(significance_Wild, "Sig.Pre-exposed", ""))), levels = c("Sig.Naive","Sig.Pre-exposed","Sig.Both"),
                 ordered = T)) %>%
  na.omit() %>%
...
A_plot <- ggplot(df, aes(x = log2FC_Wild, y = log2FC_Lab, color = Category)) +
  ...
  scale_color_manual(aesthetics = c("fill", "color"),
                     values = c("Sig.Both" = "black", 
                                "Sig.Naive" = "darkred", 
                                "Sig.Pre-exposed" = "blue"),
                                na.value = NA) +
  labs(title = "Cross_HypoxiaDEGs (Abdominal)",
    x = "log[2]FC - Pre-exposed",
    y = "log[2]FC - Naive",
    color = NULL
  ) +
  theme_bw() +
...
dr_genes <- df %>% filter((log2FC_Lab > 0 & log2FC_Wild < 0)|(log2FC_Lab < 0 & log2FC_Wild > 0)) %>%
  left_join(annot_kegg, by = "gene") %>%
  unique()
  

gene_list<-hyp_DEGs_L$gene

gc_A <- list(down_in_Naive = df[df$log2FC_Lab < 0,]$gene,
             up_in_Naive = df[df$log2FC_Lab > 0,]$gene)
...
dotplot_A <- dotplot(ck_A, showCategory = 10, label_format = 50);dotplot_A

ggsave(plot = dotplot_A,filename = "gene_expression/figures/GO_clusters_Wild_vs_Lab.pdf",width = 6,height=6)
...
A_2h <- ggplot(df[[2]][[1]], aes(x = log2FC_Wild, y = log2FC_Lab, color = Category)) +
  geom_point(alpha = 0.7, size = 1.5, shape = 1) +
  scale_color_manual(values = c("Sig.Both" = "black", 
                                "Sig.Naive" = "darkred", 
                                "Sig.Pre-exposed" = "blue"),
                                na.value = NA) +
  labs(x = "log[2]FC - Pre-exposed",
    y = "log[2]FC - Naive",
    color = NULL
  ) +
  theme_bw() +
...
gc_P <- list(down_in_Naive = df[df$log2FC_Lab < 0,]$gene,
             up_in_Naive = df[df$log2FC_Lab > 0,]$gene)
...
dotplot_P <- dotplot(ck_P, showCategory = 10, label_format = 50);dotplot_P

ggsave(plot = dotplot_P,filename = "gene_expression/figures/GO_clusters_Wild_vs_Lab_Pp.pdf",width = 6,height=6)
...
p_6d <- ggplot(df[[2]][[4]], aes(x = log2FC_Wild, y = log2FC_Lab, color = Category)) +
  ...
  scale_color_manual(values = c("Sig.Both" = "black", 
                                ...
                                "Sig.Pre-exposed" = "blue"),
                                na.value = NA) +
    geom_label_repel(data = df[[2]][[4]] %>% filter((log2FC_Lab > 1 & log2FC_Wild < -1)|(log2FC_Lab < -1 & log2FC_Wild > 1)), 
                  aes(label = Gene_names, fill = Category, segment.color = "black"), color = "white", ,
            size = 3, box.padding = 0.5, point.padding = unit(0.3, 
                "lines"), max.overlaps = Inf) +
  labs(x = "log[2]FC - Pre-exposed",
...
merged.res$type = "upregulated in lab-cross"
merged.res$type[merged.res$NES < 0] = "downregulated in lab-cross"
...
plt <-ggplot(GOs_ancestors_clust, aes(x = Cluster, y = Description, color=ancestor_name, shape=type)) +
    ...
    scale_shape_manual(values=c("\u25B2","\u25BC"),breaks = c('upregulated in lab-cross','downregulated in lab-cross'))
...
GO_reduced$type = "upregulated in lab-cross"
GO_reduced$type[GO_reduced$NES < 0] = "downregulated in lab-cross"
...
plt1 <-ggplot(GO_reduced, aes(x = Cluster, y = parentTerm, color=type, shape=type)) +
    ...
    scale_shape_manual(name = "direction", values=c("\u25B2","\u25BC"),breaks = c('upregulated in lab-cross','downregulated in lab-cross')) +
    scale_color_manual(name = "direction",breaks = c('upregulated in lab-cross','downregulated in lab-cross'), values = c("#B31B21", "#1465AC")) 
...
cairo_pdf("gene_expression/figures/FigSn_Reduced_GOBP_Abdominal.pdf", width = 13, height = 10)
plt1
dev.off()
```
...
cairo_pdf("gene_expression/figures/FigSn_KEGGenrichment_Abdominal.pdf", width = 12.5, height = 8)
plt
dev.off()
...
cairo_pdf("gene_expression/figures/FigSn_MKEGGenrichment_Abdominal.pdf", width = 12.5, height = 5)
plt
dev.off()
```
