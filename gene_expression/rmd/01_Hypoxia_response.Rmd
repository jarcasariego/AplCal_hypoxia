...
sdat0 <- read_csv("gene_expression/data/sample_metadata.csv") %>%
  dplyr::filter(sample_ID %in% sampleNames)
...
sdat_W <- sdat0[sdat0$batch == "Wild_cross",] %>%
  arrange(sample_ID) %>%                              # order rows by sample name
  column_to_rownames(var = "sample_ID")               # set sample name to rownames

countMatrix_W <- countMatrix[, colnames(countMatrix) %in% rownames(sdat_W)]

sdat_L <- sdat0[sdat0$batch == "Lab_cross",] %>%
  arrange(sample_ID) %>%                              # order rows by sample name
  column_to_rownames(var = "sample_ID") 

countMatrix_L <- countMatrix[, colnames(countMatrix) %in% rownames(sdat_L)]
```
# Wild Cohort
...
dds <- DESeqDataSetFromMatrix(countData = countMatrix_W,
                              colData = sdat_W,
...
dds <- DESeqDataSetFromMatrix(countData = countMatrix_W,
                              colData = sdat_W,
                              design = ~ treatment + tissue)
### Remove genes counted less than 1
...
# Define colors
exposure_colors <- brewer.pal(6, "Oranges")  # Gradient for exposure levels
recovery_colors <- c()  # Distinct colors for recovery

# Plot with spiders
pcoa <- ggplot(mds, aes(color = treatment, shape = tissue)) +
  geom_segment(mapping = aes(x = `1`, y = `2`, xend = c1, yend = c2),
               lwd = 0.25, col = "grey", show.legend = F) +
  geom_point(size = 3, aes(x = c1, y = c2, fill = treatment)) +
  geom_point(size = 1, aes(x = `1`, y = `2`, fill = treatment), show.legend = F) +
  geom_point(size = 1, aes(x = `1`, y = `2`), show.legend = F) +
  scale_color_manual(aesthetics = c("colour", "fill"), 
                     values = c("#1b6e99","#FDAE6B","#FD8D3C","#E6550D","#A63603","#1b9e90","#1b6e01"), 
                    name = "treatment", labels = c("Control","Hyp_2h","Hyp_6h","Hyp_5d","Hyp_6d","Recovery","Reox")) +
...
ggsave(filename = "gene_expression/figures/Pcoa_GE_Wild.png",pcoa, width = 4.5, height = 3.5)
```
...
write_csv(res_A %>%
            left_join(AcTxAnot2 %>%
                        dplyr::select(gene, Gene_names, Protein_names) %>%
                        unique(), by = "gene"), file = "gene_expression/output/DE_wild/Abdominal/DE_sig_data.csv")
...
write_csv(res_Pp %>%
            left_join(AcTxAnot2 %>%
                        dplyr::select(gene, Gene_names, Protein_names)  %>%
                        unique(), by = "gene"), file = "gene_expression/output/DE_wild/Pleural/DE_sig_data.csv")
```
...
DEGs_A_up <- list(Hyp_T2h=pull(DEGs_A[which(DEGs_A$contrast=="Hyp_T2h.Control" & DEGs_A$log2FoldChange > 0),2]),
                 Hyp_T6h=pull(DEGs_A[which(DEGs_A$contrast=="Hyp_T6h.Control" & DEGs_A$log2FoldChange > 0),2]),
                 Hyp_T6d=pull(DEGs_A[which(DEGs_A$contrast=="LtH_6.Control" & DEGs_A$log2FoldChange > 0),2]),
                 Hyp_T7d=pull(DEGs_A[which(DEGs_A$contrast=="LtH_7.Control" & DEGs_A$log2FoldChange > 0),2]))
DEGs_A_down <- list(Hyp_T2h=pull(DEGs_A[which(DEGs_A$contrast=="Hyp_T2h.Control" & DEGs_A$log2FoldChange < 0),2]),
                 Hyp_T6h=pull(DEGs_A[which(DEGs_A$contrast=="Hyp_T6h.Control" & DEGs_A$log2FoldChange < 0),2]),
                 Hyp_T6d=pull(DEGs_A[which(DEGs_A$contrast=="LtH_6.Control" & DEGs_A$log2FoldChange < 0),2]),
                 Hyp_T7d=pull(DEGs_A[which(DEGs_A$contrast=="LtH_7.Control" & DEGs_A$log2FoldChange < 0),2]))

str(DEGs_A_wide)
gv1 <- ggvenn(DEGs_A_up, fill_color = c("#0073C2FF", "#0050C5", "#00C2FF", "#0032C2FF"),
  stroke_size = 0.5, set_name_size = 4, show_percentage = F, stroke_color = F 
  )
gv1

A_up_OG <- intersect(intersect(DEGs_A[which(DEGs_A$contrast=="Hyp_T2h.Control" & DEGs_A$log2FoldChange > 0),]$gene,
          DEGs_A[which(DEGs_A$contrast=="Hyp_T6h.Control" & DEGs_A$log2FoldChange > 0),]$gene), intersect(
          DEGs_A[which(DEGs_A$contrast=="LtH_6.Control" & DEGs_A$log2FoldChange > 0),]$gene,
          DEGs_A[which(DEGs_A$contrast=="LtH_7.Control" & DEGs_A$log2FoldChange > 0),]$gene))

A_down_OG <- intersect(intersect(DEGs_A[which(DEGs_A$contrast=="Hyp_T2h.Control" & DEGs_A$log2FoldChange < 0),]$gene,
          DEGs_A[which(DEGs_A$contrast=="Hyp_T6h.Control" & DEGs_A$log2FoldChange < 0),]$gene), intersect(
          DEGs_A[which(DEGs_A$contrast=="LtH_6.Control" & DEGs_A$log2FoldChange < 0),]$gene,
          DEGs_A[which(DEGs_A$contrast=="LtH_7.Control" & DEGs_A$log2FoldChange < 0),]$gene))

df <- data.frame(gene = A_down_OG) %>%
  left_join(AcTxAnot2, by = "gene")

gv2 <- ggvenn(DEGs_A_down, fill_color = c("red", "#CD522CFF", "#CD521CFF", "#CD534CFF"),
  stroke_size = 0.5, set_name_size = 4, show_percentage = F, stroke_color = F 
  )
gv2
...
DEGs_Pp_up <- list(Hyp_T2h=pull(DEGs_Pp[which(DEGs_Pp$contrast=="Hyp_T2h.Control" & DEGs_Pp$log2FoldChange > 0),2]),
                 Hyp_T6h=pull(DEGs_Pp[which(DEGs_Pp$contrast=="Hyp_T6h.Control" & DEGs_Pp$log2FoldChange > 0),2]),
                 Hyp_T6d=pull(DEGs_Pp[which(DEGs_Pp$contrast=="LtH_6.Control" & DEGs_Pp$log2FoldChange > 0),2]),
                 Hyp_T7d=pull(DEGs_Pp[which(DEGs_Pp$contrast=="LtH_7.Control" & DEGs_Pp$log2FoldChange > 0),2]))
DEGs_Pp_down <- list(Hyp_T2h=pull(DEGs_Pp[which(DEGs_Pp$contrast=="Hyp_T2h.Control" & DEGs_Pp$log2FoldChange > 0),2]),
                 Hyp_T6h=pull(DEGs_Pp[which(DEGs_Pp$contrast=="Hyp_T6h.Control" & DEGs_Pp$log2FoldChange < 0),2]),
                 Hyp_T6d=pull(DEGs_Pp[which(DEGs_Pp$contrast=="LtH_6.Control" & DEGs_Pp$log2FoldChange < 0),2]),
                 Hyp_T7d=pull(DEGs_Pp[which(DEGs_Pp$contrast=="LtH_7.Control" & DEGs_Pp$log2FoldChange < 0),2]))
...
str(DEGs_Pp_wide)
gv1 <- ggvenn(DEGs_Pp_up, fill_color = c("#0073C2FF", "#0050C5", "#00C2FF", "#0032C2FF"),
  stroke_size = 0.5, set_name_size = 4, show_percentage = F, stroke_color = F 
  )
gv1

gv2 <- ggvenn(DEGs_Pp_down, fill_color = c("red", "#CD522CFF", "#CD521CFF", "#CD534CFF"),
  stroke_size = 0.5, set_name_size = 4, show_percentage = F, stroke_color = F 
  )
gv2

gv3 <- ggvenn(DEGs_Pp_wide, fill_color = c("#0073C2FF", "#EFC000FF", "#868686FF", "#CD534CFF"),
  stroke_size = 0.5, set_name_size = 4, show_percentage = F, stroke_color = F 
  )
gv3
...
DEGs_wild_wide <- list(abdominal=pull(DEGs_wild[which(DEGs_wild$tissue=="A" ),1]),
                 pleural=pull(DEGs_wild[which(DEGs_wild$tissue=="Pp"),1]))
DEGs_wild_up <- list(abdominal=pull(DEGs_wild[which(DEGs_wild$tissue=="A" & DEGs_wild$log2FoldChange > 0),1]),
                 pleural=pull(DEGs_wild[which(DEGs_wild$tissue=="Pp" & DEGs_wild$log2FoldChange > 0),1]))
DEGs_wild_down <- list(abdominal=pull(DEGs_wild[which(DEGs_wild$tissue=="A" & DEGs_wild$log2FoldChange < 0),1]),
                 pleural=pull(DEGs_wild[which(DEGs_wild$tissue=="Pp" & DEGs_wild$log2FoldChange < 0),1]))
str(DEGs_wild_wide)
gv1 <- ggvenn(DEGs_wild_up, fill_color = c("red", "#CD522CFF"),
  stroke_size = 0.5, set_name_size = 4, show_percentage = F, stroke_color = F 
  )
gv1
gv2 <- ggvenn(DEGs_wild_down, fill_color = c("#0073C2FF", "#0050C5"),
  stroke_size = 0.5, set_name_size = 4, show_percentage = F, stroke_color = F 
  )
gv2
...
DEGs_wild_wide <- list(abdominal=unique(pull(DEGs_wild[which(DEGs_wild$tissue=="A"),2])),
                 pleural=unique(pull(DEGs_wild[which(DEGs_wild$tissue=="Pp"),2])))

cds <- DESeqDataSetFromMatrix(countMatrix_W, sdat_W, design = ~treatment + tissue)
...
res_patterns_A <- DEGreport::degPatterns(
  ma = normalized_L2[rownames(normalized_L2) %in% DEGs_wild_wide$abdominal, rownames(sdat_W)],
  metadata = data.frame(time = sdat_W$treatment, row.names = rownames(sdat_W), Tissue = sdat_W$tissue),
  reduce = TRUE,
  minc = 50,
  col = "Tissue",
...
res_patterns_Pp <- DEGreport::degPatterns(
  ma = normalized_L2[rownames(normalized_L2) %in% DEGs_wild_wide$pleural, rownames(sdat_W)],
  metadata = data.frame(time = sdat_W$treatment, row.names = rownames(sdat_W),
                        Tissue = sdat_W$tissue), 
  reduce = TRUE,
  minc = 20,
  col = "Tissue",
  groupDifference = 0.58)
...
load(file ="gene_expression/data/clusters_Pp.R")
...
draw(lgd, x = unit(5, "cm"), y = unit(5, "cm"))
...
# Lab Cohort
### Create DESeq objects
```{r create_dds Lab_cross}

# Create full DESeqDataSet
dds <- DESeqDataSetFromMatrix(countData = countMatrix_L,
                              colData = sdat_L,
                              design = ~ treatment + tissue)
### Remove genes counted zero times
dds <- dds[ rowSums(counts(dds)) > 0, ]
...
# AC163A seems like an outlier in pca but groups with other control samples in correlation. I will maintain
```
...
ggsave(filename = "gene_expression/figures/Pcoa_GE_lab.png",pcoa, width = 4.5, height = 3.5)
```

```{r subset count data for TS analysis Lab_cross}

dir.create(paste0('gene_expression/data/','Lab_PlPed_vs_Abd'))
...
sdat1 <- sdat0 %>%
  ...
  ungroup() %>%
  dplyr::select(sample, replicate, timepoint, group, treatment)
write_csv(sdat1, "gene_expression/data/Lab_PlPed_vs_Abd/sample_meta.csv")
  

# Directory where files will be saved
dir.create(paste0('gene_expression/data/','Lab_PlPed_vs_Abd/counts'))
output_dir <- "/gene_expression/data/Lab_PlPed_vs_Abd/counts/"  # Specify your directory
...
```{r pipeline for DE and clustering PluralvsAbdominal_lab}

#Give names to saved object and name of results repository
dir.create('gene_expression/output/TS_output')
dir.create('gene_expression/output/TS_output/Lab_PlPed_vs_Abd')
name_result_folder<-'gene_expression/output/TS_output/Lab_PlPed_vs_Abd/'
obj_name<-'timeSeries_obj_Lab_PlPed_vs_Abd.Rdata'
...
TS_object <- add_experiment_data(TS_object,
                                        sample_dta_path="gene_expression/data/Lab_PlPed_vs_Abd/sample_meta.csv",
                                        count_dta_path="gene_expression/data/Lab_PlPed_vs_Abd/counts/")
TS_object <- add_semantic_similarity_data(TS_object,"BP")
...
### Get significant DEGs
```{r get_DEgenes Abdominal all Lab}
DE <- DE %>%
  mutate(sig = map(dsr, ~ rownames_to_column(data.frame(.[which(.$padj < 0.05), ]), "gene")),
          up = map(sig, ~ filter(., log2FoldChange > 0)),
        down = map(sig, ~ filter(., log2FoldChange < 0)),
...
```{r export complete matrix with DE results for CtrlvsHypoxia contrasts Lab}
DE %>%
  unnest(full) %>%
  filter(tissue == "A") %>%
  mutate(contrast = paste(num, den, sep = ".")) %>%
  filter(contrast %in% c("Hyp_T2h.Control", "Hyp_T6h.Control", 
                         ...
                         "Recovery.Hyp_T6h", "Recovery.Control")) %>%
  dplyr::select(., gene, log2FoldChange, padj, contrast) %>%
  write_csv(., file = "gene_expression/output/DE_lab/Abdominal/DE_raw_data.csv")
...
DE %>%
  ...
  filter(contrast %in% c("Hyp_T2h.Control", "Hyp_T6h.Control", 
                         ...
                         "Recovery.Hyp_T6h", "Recovery.Control")) %>%
  dplyr::select(., gene, log2FoldChange, padj, contrast) %>%
  write_csv(., file = "gene_expression/output/DE_lab/Pleural/DE_raw_data.csv")
...
write_csv(res_A %>%
            left_join(AcTxAnot2 %>%
                        dplyr::select(gene, Gene_names, Protein_names) %>%
                        unique(), by = "gene"), file = "gene_expression/output/DE_lab/Abdominal/DE_sig_data.csv")
...
write_csv(res_Pp %>%
            left_join(AcTxAnot2 %>%
                        dplyr::select(gene, Gene_names, Protein_names) %>%
                        unique(), by = "gene"), file = "gene_expression/output/DE_lab/Pleural/DE_sig_data.csv")
```


```{r plot DEG results Lab}
DEGs_A <- DE %>%
  unnest(sig) %>%
  filter(tissue == "A") %>%
  mutate(contrast = paste(num, den, sep = ".")) %>%
...
ggsave("gene_expression/output/DE_lab/Abdominal/DEGs_vennDiagram.png", gv3, height = 3, width = 5)
...
gv3
ggsave("gene_expression/output/DE_lab/Pleural/DEGs_vennDiagram.png", gv2, height = 3, width = 5)
...
DEGs_lab_wide <- list(abdominal=pull(DEGs_lab[which(DEGs_lab$tissue=="A" ),1]),
                 pleural=pull(DEGs_lab[which(DEGs_lab$tissue=="Pp"),1]))
DEGs_lab_up <- list(abdominal=pull(DEGs_lab[which(DEGs_lab$tissue=="A" & DEGs_lab$log2FoldChange > 0),1]),
                 pleural=pull(DEGs_lab[which(DEGs_lab$tissue=="Pp" & DEGs_lab$log2FoldChange > 0),1]))
DEGs_lab_down <- list(abdominal=pull(DEGs_lab[which(DEGs_lab$tissue=="A" & DEGs_lab$log2FoldChange < 0),1]),
                 pleural=pull(DEGs_lab[which(DEGs_lab$tissue=="Pp" & DEGs_lab$log2FoldChange < 0),1]))
str(DEGs_lab_wide)
gv1 <- ggvenn(DEGs_lab_up, fill_color = c("red", "#CD522CFF"),
  stroke_size = 0.5, set_name_size = 4, show_percentage = F, stroke_color = F 
  )
gv1
gv2 <- ggvenn(DEGs_lab_down, fill_color = c("#0073C2FF", "#0050C5"),
  stroke_size = 0.5, set_name_size = 4, show_percentage = F, stroke_color = F 
  )
gv2
gv3 <- ggvenn(DEGs_lab_wide, fill_color = c("#0073C2FF", "#EFC000FF"),
  stroke_size = 0.5, set_name_size = 4, show_percentage = F, stroke_color = F 
  )
gv3
ggsave("gene_expression/output/DE_lab/DEGs_Lab_by_tissue_vennDiagram.png", gv3, height = 3, width = 5)
...
# Save all plots to files
volcano_plots_A %>%
  mutate(plot_paths = map2(plots, contrast, ~ ggsave(filename = paste0("gene_expression/output/DE_lab/Abdominal/",.y, "_volcano.png"), plot = .x)))
...
# Save all plots to files
volcano_plots_Pp %>%
  mutate(plot_paths = map2(plots, contrast, ~ ggsave(filename = paste0("gene_expression/output/DE_lab/Pleural/",.y, "_volcano.png"), plot = .x)))
...
```{r cluster transcriptional profiles Lab, echo=FALSE, message=FALSE, warning = FALSE} 
...
DEGs_lab <- DE %>%
  ...
  dplyr::select(.,contrast, gene, tissue, log2FoldChange) %>%
  filter(abs(log2FoldChange) >= 0)
DEGs_lab_wide <- list(abdominal=pull(DEGs_lab[which(DEGs_lab$tissue=="A"),2]),
                 pleural=pull(DEGs_lab[which(DEGs_lab$tissue=="Pp"),2]))

cds <- DESeqDataSetFromMatrix(countMatrix_L, sdat_L, design = ~tissue.group)
cds <- cds[ rowMeans(counts(cds)) > 1, ]
dds <- estimateSizeFactors(cds)
normalized_counts <- counts(dds, normalized=TRUE)
...
# use all DEGs for temporal contrasts
res_patterns_A <- DEGreport::degPatterns(
  ma = normalized_L2[rownames(normalized_L2) %in% DEGs_lab_wide$abdominal, rownames(sdat_L)],
  metadata = data.frame(time = sdat_L$treatment, row.names = rownames(sdat_L), Tissue = sdat_L$tissue),
  reduce = TRUE,
  minc = 70,
  col = "Tissue",
  groupDifference = 0.58
...
res_patterns_Pp <- DEGreport::degPatterns(
  ma = normalized_L2[rownames(normalized_L2) %in% DEGs_lab_wide$pleural, rownames(sdat_L)],
  metadata = data.frame(time = sdat_L$treatment, row.names = rownames(sdat_L),
                        Tissue = sdat_L$tissue), 
  reduce = TRUE,
  minc = 50,
  col = "Tissue",
  groupDifference = 0.58)
...
A.norm <- res.A_patterns %>% 
  ...
  dplyr::select(genes, time, value, cluster) %>% 
  tidyr::spread(time, value) %>% column_to_rownames("genes")
A.norm[A.norm$cluster == 6,]$cluster <- "A7"
A.norm[A.norm$cluster == 11,]$cluster <- "A8"
A.norm[A.norm$cluster == 12,]$cluster <- "A9"
A.norm[A.norm$cluster == 15,]$cluster <- "A1"
A.norm[A.norm$cluster == 17,]$cluster <- "A2"
A.norm[A.norm$cluster == 53,]$cluster <- "A3"
A.norm[A.norm$cluster == 61,]$cluster <- "A10"
A.norm[A.norm$cluster == 73,]$cluster <- "A4"
A.norm[A.norm$cluster == 74,]$cluster <- "A11"
A.norm[A.norm$cluster == 88,]$cluster <- "A12"
A.norm[A.norm$cluster == 100,]$cluster <- "A5"
A.norm[A.norm$cluster == 114,]$cluster <- "A6"
A.norm <- A.norm[c(1:4,8,5:7)]

# A1-A6 Up, A7-A9 Down 

A_clust_UP <- c("A1","A2","A3","A4","A5","A6")
A_clust_DOWN <- c("A7","A8","A9","A10","A11","A12")
...
res.A_patterns$clust <- factor(A_clust[as.character(res.A_patterns$cluster)], levels = c("A1","A2","A3","A4","A5",
                                                                                      ...
                                                                                      "A11","A12"),
                               ordered = T)
A.norm$cluster <- factor(A.norm$cluster, levels = c("A1","A2","A3","A4","A5","A6","A7","A8","A9","A10","A11","A12"), ordered = T)
res.A_clusters$clust <- factor(A_clust[as.character(res.A_clusters$cluster)], levels = c("A1","A2","A3","A4","A5","A6","A7","A8","A9","A10","A11","A12"), ordered = T)
res.A_clusters <- res.A_clusters %>% mutate( direction = 
                               ifelse(clust %in% c("A7","A8","A9","A10","A11","A12"), "DOWN","UP")
                               ) 

A_clusters <- split(res.A_clusters, f = res.A_clusters$clust)
names(A_clusters) <- unique(res.A_clusters$clust)

save(res.A_clusters,res.A_patterns, file ="gene_expression/data/Lab_clusters_A.R")
###Pp heatmap 
Pp.norm <- res.Pp_patterns %>% 
  filter(.,Tissue == "Pp") %>%
  dplyr::select(genes, time, value, cluster) %>% 
  tidyr::spread(time, value) %>% column_to_rownames("genes")
Pp.norm[Pp.norm$cluster == 3,]$cluster <- "P5"
Pp.norm[Pp.norm$cluster == 4,]$cluster <- "P4"
Pp.norm[Pp.norm$cluster == 18,]$cluster <- "P1"
Pp.norm[Pp.norm$cluster == 29,]$cluster <- "P2"
Pp.norm[Pp.norm$cluster == 32,]$cluster <- "P3"
Pp.norm <- Pp.norm[c(1:4,8,5:7)]
...
res.Pp_clusters$clust <- Pp_clust[as.character(res.Pp_clusters$cluster)]
res.Pp_clusters <- res.Pp_clusters %>% mutate( direction = 
                               ifelse(clust %in% c("P5", "P4"),"DOWN", "UP")
                               ) 
Pp_clusters <- split(res.Pp_clusters, f = res.Pp_clusters$clust)
names(Pp_clusters) <- unique(res.Pp_clusters$clust)

save(res.Pp_clusters,res.Pp_patterns, file ="gene_expression/data/Lab_clusters_Pp.R")
```
...
# svg(filename = "../figures/A.clusters.svg", width = 3.625, height = 8)
#pdf(file = "gene_expression/figures/Lab_A.clusters.pdf", width = 4, height = 8)
draw(ht.A.clusters, heatmap_height = unit(19.5, "cm"), heatmap_legend_side = "bottom",  padding = unit(c(0, 0, 0, 0.5), "cm"))
...
# svg(filename = "../figures/Pp.clusters.svg", width = 3.625, height = 8)
pdf(file = "gene_expression/figures/Lab_Pp.clusters.pdf", width = 4, height = 5)
draw(ht.Pp.clusters, heatmap_height = unit(7.5, "cm"), heatmap_legend_side = "bottom",  padding = unit(c(0, 0, 0, 0.5), "cm"))
# decorate_row_title("scaled_TPM",{
#     grid.text("B", y = unit(1.6, "npc") , x = unit(0, "npc"), just = "left", gp = gpar(fontsize = 22)) })
