...
res_A <- DE %>%
  ...
  filter(contrast %in% c("Hyp_T2h.Control", "Hyp_T6h.Control", 
                         ...
                         "Recovery.Hyp_T6h", "Recovery.Control")) %>%
  dplyr::select(.,contrast, gene, log2FoldChange)
write_csv(res_A, file = "gene_expression/output/DE_lab/Abdominal/DE_sig_data.csv")
...
res_Pp <- DE %>%
  ...
  filter(contrast %in% c("Hyp_T2h.Control", "Hyp_T6h.Control", 
                         ...
                         "Recovery.Hyp_T6h", "Recovery.Control")) %>%
  dplyr::select(.,contrast, gene, log2FoldChange)
write_csv(res_A, file = "gene_expression/output/DE_lab/Pleural/DE_sig_data.csv")
```
...
A.norm <- res.A_patterns %>% 
  ...
  dplyr::select(genes, time, value, cluster) %>% 
  tidyr::spread(time, value) %>% column_to_rownames("genes")
A.norm[A.norm$cluster == 6,]$cluster <- "A1"
A.norm[A.norm$cluster == 11,]$cluster <- "A2"
A.norm[A.norm$cluster == 12,]$cluster <- "A3"
A.norm[A.norm$cluster == 15,]$cluster <- "A7"
A.norm[A.norm$cluster == 17,]$cluster <- "A8"
A.norm[A.norm$cluster == 53,]$cluster <- "A9"
A.norm[A.norm$cluster == 61,]$cluster <- "A4"
A.norm[A.norm$cluster == 73,]$cluster <- "A10"
A.norm[A.norm$cluster == 74,]$cluster <- "A5"
A.norm[A.norm$cluster == 88,]$cluster <- "A6"
A.norm[A.norm$cluster == 100,]$cluster <- "A11"
A.norm[A.norm$cluster == 114,]$cluster <- "A12"
A.norm <- A.norm[c(1:4,8,5:7)]

# A1-A6 Up, A7-A9 Down 

A_clust_UP <- c("A1","A2","A3","A4","A5","A6")
A_clust_DOWN <- c("A7","A8","A9","A10","A11","A12","A13")
